function PannelPlotPwclim (yr,Dom,lambda)
if strcmp(Dom,'GB')
...
f=load(flnm);
%f=f-min(f); 
F=csvreadK([direct,'totalRF.csv']); f=stdOL(F,.3);
patch(xg(E')+xs,yg(E')+ys,(1*f(E')).^lambda);shading interp;colormap('jet');caxis(cax);
