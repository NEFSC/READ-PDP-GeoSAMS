...
%                   this function /PRN)
  ...
  if (isempty (ext))
    bname = fname;
    fname = strcat(fname, '.shp');
  elseif (isempty (pth))
    % Later on bname.shx and bname.dbf will be read
    bname = fnm;
  else
    % Later on bname.shx and bname.dbf will be read
    bname = strcat(pth, filesep, fnm);
  end
  ...
  for ii = 1:2:length (varargin)
    ...
    elseif (numel (varargin{ii}) < 3)
      ...
      varargin_str = lower (varargin{ii});
      switch (varargin_str(1:3))
        case "att"
        ...
        case "rec"
          ...
          try
            ...
            if (have_shx && any (s_recs > nrec))
              fprintf (["shaperead: requested record nos. > nr. of records ", ...
                       "(%d) ignored\n"], nrec);
        ...
        case "sel"
          ...
          fprintf ("shaperead: 'Selector' option not implemented, option ignored\n");
        case "use"
  ...
  if (clip)
    ...
    if (isempty (which ("clipPolygon_clipper")))
      ...
      fprintf  ("shaperead: function 'clipPolygon' not found. Clip option ignored\n");
      warning ("           (OF geometry package installed and loaded?)\n");
      clip = 0;
    end
    if (isempty (which ("distancePoints")))
      % No OF geometry package?
      fprintf  ("shaperead: function 'distancePoints' not found. Clip option ignored\n");
      warning ("           (OF geometry package installed and loaded?)\n");
      clip = 0;
    end
  end
  ...
  while 1 %do
    ...
    if (isempty (s_recs) || ismember (double (val(5)), s_recs))
      ...
      if (any (abs (val(:, 1:2)) > 1.797e308))
        ...
        rincl = 0;
        if (dbug)
          fprintf ("shape# %d has no finite XY coordinates, skipped\n", ir);
        end
        ...
        switch outopts
          case {0, 1}
            ...
            if (ign_mz && val(1, 6) >= 10)
              fprintf ("shaperead: M and Z values ignored for ml-style output\n");
              ign_mz = 0;
            end
            switch val(1, 6)
              case {1, 11, 21}                              % Point
...
%             end
    ...
    ir = ir + 1;
     ...
     if (ftell (fidp) > flngt - 3 || (have_shx && ir > numel (s_recs)))
        break;
     end
  % i.e., within last
  % file bytes or all
  ...
  if (isempty (which ("dbfread")))
    fprintf ("shaperead: dbfread function not found. No attributes will be added.\n");
    fprintf ("             (io package installed and loaded?)\n");
    oatt = {};

  else
    % Try to read the .dbf file
    try
      atts = dbfread (strcat(bname, '.dbf'), s_recs, s_atts);
      if (~ isempty (atts))
        if (outopts < 2)
          ...
          if (nargout < 2)
            ...
            for ii=1:size (atts, 2)
              atts_deal = deal (atts(2:end, ii));
              [outs.(atts{1, ii})] = atts_deal{:};
            end
            oatt = [];
          else
      ...
      fprintf ("shaperead: file %s couldn't be read;\n", strcat(bname, '.dbf'));
      fprintf ("           no attributes appended\n");
            end % try
  end
