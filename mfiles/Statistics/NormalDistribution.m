...
classdef NormalDistribution
  ...
  methods (Hidden)
    ...
    function display_ (this)
      fprintf ("%s =\n", inputname(1));
      disp (this, "normal distribution");
    end
  ...
  methods (Access = public)
    ...
    function p = cdf (this, x, uflag)
      ...
      if (this.IsTruncated)
        ...
        p(lb) = 0;
        p(ub) = 1;
        p(~ (lb | ub)) = p(~ (lb | ub)) - normcdf (lx, this.mu, this.sigma);
        p(~ (lb | ub)) = p(~ (lb | ub)) / diff (normcdf ([lx, ux], this.mu, this.sigma));
      end
      % Apply uflag
      if (utail)
        p = 1 - p;
    ...
    function y = pdf (this, x)
      ...
      if (this.IsTruncated)
        ...
        ub = x > ux;
        y(lb | ub) = 0;
        y(~ (lb | ub)) = y(~ (lb | ub)) / diff (normcdf ([lx, ux], this.mu, this.sigma));
      end
    end
    ...
    %
    % @end deftypefn
    function [varargout] = plot_(this, varargin)
      if (~ isscalar (this))
        error ("plot: requires a scalar probability distribution.");
      end
      h = plot(this, false, varargin{:});
      if (nargout > 0)
        varargout{1} = h;
      end
    end
    ...
    function [varargout] = proflik (this, pnum, varargin)
      ...
      [varargout{1:nargout}] = proflik__ (this, pnum, varargin{:});
    end
