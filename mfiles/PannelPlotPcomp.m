function PannelPlotPcomp (yr,Dom,lambda)
if strcmp(Dom,'GB')
...
%close all;
clf;
directc=['Output/Sim',Dom,'Clim',int2str(yr),'/'];
%direct=['Output/Sim',Dom,'Clim',int2str(yr),'nc/'];
%direct=['Output/Sim',Dom,'lin',int2str(yr),'/'];
direct=['Output/Sim',Dom,int2str(yr),'/'];
...
cax=[0,max(f.^lambda)]
colorscatterp(x,y,fo.^lambda,6,5000);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X,G(k).Y,'k');end
text(mean(xg)+xst,max(yg)+yst,'data');

flnm=[direct,'KrigingEstimate.txt'];
f0=load(flnm);
patch(xg(E')+xs,yg(E'),f0(E').^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs,G(k).Y,'k');end
text(mean(xg)+xst+xs,max(yg)+yst,'krig field');

flnm=[directc,'KrigingEstimate.txt'];
fc=load(flnm);
patch(xg(E')+xs+xs,yg(E'),fc(E').^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs+xs,G(k).Y,'k');end
text(mean(xg)+xst+xs+xs,max(yg)+yst,'krig field w clim');

g=fc.^lambda-f0.^lambda;
cax
gs=cax(1) + (cax(2)-cax(1))  *(g-min(g)) /( max(g)-min(g));
gs=cax(1) + (cax(2)/2-cax(1))  *(g-min(g)) /( -min(g));
min(gs),max(gs)
patch(xg(E'),yg(E')+ys,gs(E'));shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X,G(k).Y+ys,'k');end
text(mean(xg)+xst,max(yg)+yst+ys,'fc- f0');

flnm=[direct,'SpatialTrend.txt'];
ft=load(flnm);%ft=ft-mean(ft)+mean(f)

patch(xg(E')+xs,yg(E')+ys,ft(E').^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs,G(k).Y+ys,'k');end
text(mean(xg)+xst+xs,max(yg)+yst+ys,'trend');

flnm=[directc,'SpatialTrend.txt'];
f=load(flnm);
%f=f-min(f);
patch(xg(E')+xs+xs,yg(E')+ys,(f(E')).^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs+xs,G(k).Y+ys,'k');end
text(mean(xg)+xst+xs+xs,max(yg)+yst+ys,'trend w. clim');
