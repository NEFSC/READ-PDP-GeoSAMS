function PannelPlotHabcamP (yr,Dom,lambda)
if strcmp(Dom,'GB')
...
if strcmp(Dom,'MA')
    ...
    cax=[-4,-.5];
close all;
direct=['Output/SimHabcam',Dom,int2str(yr),'/'];
%direct=['Output/Sim',Dom,'Clim',int2str(yr),'/'];

A=10.^-1

flnm=['Data/HabCamData',int2str(yr),Dom,'.csv']
F=csvreadK(flnm);
x=F(:,2);y=F(:,3);f=F(:,end);

cax=[min(log10(A)),max(log10(A+f))]
colorscatterp(x,y,log10(A+f),6,2500);shading interp;colormap('jet');caxis(cax);
...
patch(xg(E'),yg(E')+ys,log10(A+ft(E')));shading interp;colormap('jet');caxis(cax);
...
F=csvread(flnm);
fsig=std(F')';
flnm=[direct,'trendRF.csv'];
F=csvread(flnm);
fsigT=std(F')';
flnm=[direct,'noiseRF.csv'];
F=csvread(flnm);
fsigN=std(F')';

flnm=[direct,'KrigSTD.txt'];
%fsig=load(flnm);
patch(xg(E')+xs,yg(E')+ys,log10(A+fsig(E')));shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs,G(k).Y+ys,'k');end
text(mean(xg)+xst+xs,max(yg)+yst+ys,' \sigma');
...
set(gca,'visible','off');
set(h,'xtick',log10(vtick));
set(h,'xticklabel',floor(vtick));
flnm=['ColorbarIntpH',Dom,'.jpg']
eval(['print -djpeg ',flnm]);
system(['convert -trim ',flnm,' ',flnm]);
...
set(gca,'visible','off');
set(h,'ytick',log10(vtick));
set(h,'yticklabel',floor(vtick));
