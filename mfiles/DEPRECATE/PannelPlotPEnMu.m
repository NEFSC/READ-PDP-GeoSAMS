function PannelPlotP (yr,Dom,lambda)
if strcmp(Dom,'GB')
...
cax=[0,max(fo.^lambda)]*.75
%cax=[0,max(f.^lambda)]
[fot,j]=sort(fo);
colorscatterp(x(j),y(j),fo(j).^lambda,6,5000);shading interp;colormap('jet');caxis(cax);
...
feps=load(flnm);
flnm=[direct,'totalRF.csv'];
F=csvreadK(flnm);
fsig=std(F')';
fmu=mean(F')';

%f=f-min(f);
patch(xg(E')+xs,yg(E')+ys,(fsig(E')).^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs,G(k).Y+ys,'k');end
text(mean(xg)+xst+xs,max(yg)+yst+ys,'2 \sigma');

%patch(xg(E')+xs+xs,yg(E'),(fmu(E')).^lambda);shading interp;colormap('jet');caxis(cax);
%hold on;for k=1:N,plot(G(k).X+xs+xs,G(k).Y,'k');end
%text(mean(xg)+xst+xs+xs,max(yg)+yst,'Ens mean');

f=F(:,1);
patch(xg(E')+xs+xs,yg(E'),(f(E')).^lambda);shading interp;colormap('jet');caxis(cax);
hold on;for k=1:N,plot(G(k).X+xs+xs,G(k).Y,'k');end
text(mean(xg)+xst+xs+xs,max(yg)+yst,'Ens 1');
%patch(xg(E')+xs+xs,yg(E'),(2*feps(E')).^lambda);shading interp;colormap('jet');caxis(cax);
%hold on;for k=1:N,plot(G(k).X+xs+xs,G(k).Y,'k');end
%text(mean(xg)+xst+xs+xs,max(yg)+yst,'2 \sigma eps');
...
text(mean(xg)+xst+xs+xs,max(yg)+yst+ys,'Ens 2');
