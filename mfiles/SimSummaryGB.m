...
function SimSummaryGB(direct,StartYr,StopYr)
%Dom='GB';Dir='./';StartYr=2000;StopYr=2025;  Dom='GB';direct='./';StartYr=2000;StopYr=2025;
Dom='GB'
 g=1862.^2
if (Dom=='MA') 
...
cax=[0,.5];
 for yr=StartYr:StopYr,
   ...
   set(gca,'visible','off')
    ...
    h=colorbar('north')
     set(h,'position',[.5,.2,.33,.025])
   flnm=[direct,'F',int2str(yr),'.jpg']
...
cax=cax*towarea
 for yr=StartYr:StopYr,
    USD=load([direct,'Dollars',int2str(yr),'.txt'])/10^5;
    clf;
    patch(xg(E'),yg(E'),USD(E')*towarea);shading interp;colormap('jet');
    hold on;for k=1:N,plot(G(k).X,G(k).Y,'k');end;colormap('jet');caxis(cax)

    text(min(xg),max(yg)-10000,['100k$ Per Tow ',int2str(yr)])
   set(gca,'visible','off')
    d=daspect();daspect([max(d),max(d),1]);
    h=colorbar('north')
     set(h,'position',[.5,.2,.33,.025])
