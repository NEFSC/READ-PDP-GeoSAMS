...
if all(df['PARAM']==0):
    ...
    for i in region:
        ...
        if sum(df_temp['GAMPREDICTM2'])==0:
            df_grid_temp = df_grid_temp.assign(KRIGEPREDICTM2=0,
                                            KRIGEVAR=0)  # assign 0s back to the temp file
            df_grid_result = pd.concat([df_grid_result, df_grid_temp],
                                    ignore_index=True)  # concat the outputs from different regions
        else:
            ...
            df_grid_temp = df_grid_temp.assign(KRIGEPREDICTM2 = z_interp,
                                            KRIGEVAR = ss) #assign kriging results back to the temp file
            df_grid_result = pd.concat([df_grid_result, df_grid_temp], ignore_index=True) #concat the outputs from different regions
    ...
    # print(df_grid_result.groupby('NEWSAMS').agg({'FINALPREDICT':'sum'})) #print sum by region
