...
!> files stored in directories KrigingEstimates/Sim<domain name><year>/KrigingEstimate.txt  
!> 
...
!> such as: "KrigingEstimates/Sim<domain name><year>/RandomFieldxx.txt" contain @f$n_{nodes}@f$ vectors of independent 
!> random fields conditioned on survey observations from "Data/Recruits<year><domain name>.csv", 
...
!> stationary random process with semi-variogram $\gamma(s)$. For a summary of UK see Cressie 1993, pages 151 -180.
...
!> Here $z$ is bathymetric depth. We start by fitting nonlinear parameters $\lambda^{c,s}$ and $x_0^{c,s}$ to log recruitment for "cross shelf" structure.  %The non linear fitting is done with standard linear regression. i.e.
!> @f[
...
module Recruit_Mod
    ...
    subroutine Set_Recruitment(recruit, n_grids, domain_name, domain_area, element_area, &
        ...
        do year = 1979, 2018      !! TODO make these variable
            ...
            if (is_random_rec) then
                call Random_Recruits(tmp,year,year,100)
            else
        ...
        do year = 2019,2025
            ...
            call Random_Recruits(tmp,1979,2018,100)
            do j = 1,num_grids
                recruit(j)%recruitment(year_index) = tmp(j)
                recruit(j)%year(year_index) = year
                recruit(j)%rec_start = 1.D0/365.D0
    ...
    !==================================================================================================================----------------------------
    subroutine Random_Recruits(R, start_year, stop_year, num_sim)
    ...
    !> @f{eqnarray*}{
    !> M_{juv} &=& e^{(-9.701 + 1.093 Rec)} \mbox{, if } Rec > 1400 million \mbox{2030?} \\
    !>         &=& 0.25                     \mbox{        , otherwise}
    !> @f}
    !> 
    !> 
    !> I have a similar formula for GB Open:
    !> @f{eqnarray*}{
    !> M &=& e^{(1.226*log(Rec)-10.49)} \mbox{, if } Rec > 1400 million \mbox{2030?} \\
    !>         &=& 0.2                     \mbox{        , otherwise}
    !> @f}
    !>
